...
class NearestNeighbourAlgorithm : public OptimizationAlgorithm<SOL, TYPE_FITNESS, TYPE_CELL> {
    public:
    NearestNeighbourAlgorithm(std::mt19937 &mt_rand, 
    ...
    OptimizationAlgorithm<SOL, TYPE_FITNESS, TYPE_CELL>(mt_rand, std::move(statistic), std::move(stoppingCriteria), problem) {
        BOOST_LOG_TRIVIAL(debug) << __FILE__ << ":"<<__LINE__<<" Creation NearestNeighbourAlgorithm";
    }
    ...
    std::unique_ptr<SOL> operator()(const SOL &s) {
        ...
        SOL subsolution(2);
        rid = std::make_unique<std::uniform_int_distribution<unsigned int>>(0, solution_final->sizeArray()-1);
        std::pair<std::vector<unsigned int>::iterator, double> best_node; // <id_node, fitness>

        // 
        std::vector<unsigned int> nodes_not_use;
        for(unsigned int i = 0 ; i < solution_final->sizeArray() ; i++)
            nodes_not_use.push_back(i);
        ...
        for(unsigned int i = 1 ; i < solution_final->sizeArray() ; i++) {
            ...
            for(std::vector<unsigned int>::iterator it=nodes_not_use.begin() + 1; it!=nodes_not_use.end(); ++it) {
            ...
            std::cout<<count<<" "<< (solution_final->sizeArray() * (solution_final->sizeArray() - 1))/2 <<"\r";
            std::cout.flush();
        }
